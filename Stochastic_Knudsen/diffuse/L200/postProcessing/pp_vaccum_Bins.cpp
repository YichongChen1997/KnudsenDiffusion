...
int main()
{
    ...
    for (t = 0; t < nTimeSteps; t++)
    {
        ...
        if (t == 0)
        {
            ...
            Lx = xDelta;              // average the channel region
            Ly = yHi - yLo;
            Lz = zHi - zLo;
        ...
        for (n = 0; n < nAtoms; n++)
        {
            ...
            if (t >= skipTimeStep)
            {
                if ((x >= xDelta + xLf + xL) && (x <= xHi)) // within the vaccum
                {
                    if ((z >= zLo) && (z <= zHi))
                    {
    ...
    {
        ofstream binFile1("bins_vaccum_coarse.txt", ios::out);
    ...
    {
        ofstream binFile2("bins_vaccum_fine.txt", ios::out);
