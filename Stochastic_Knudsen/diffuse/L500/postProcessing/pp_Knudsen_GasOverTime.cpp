...
int main()
{
    ...
    ofstream file("channel_gasOverTime.txt", ios::out);
    ...
    for (t = 0; t < nTimeSteps; t++)
    {
        ...
        // Mean Velocity (m/s)
        vMeanGas /= double(countChannelMol);

        // Kinetic Energy
        keGas = mGas * (keGasTemp - vMeanGas * vMeanGas * countChannelMol);

        // Temperature of gas (K)
        tempGas = keGas * refVelocity * refVelocity / (3.0 * kB * countChannelMol);
